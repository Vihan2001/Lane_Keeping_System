...
p_controller1 = PIDController(kp=0.8, ki=0.001, kd=0.4, ts=dt)
y_cache = [audi.y]
t_cache = [0]
u_dist = np.deg2rad(1)
...
# kp = 0.1, Kd = 0.1, Ki =
audi = Car(length=2.3, y=0.3)
p_controller2 = PIDController(kp=0.8, ki=0.01, kd=0.4, ts=dt)
y_cache2 = [audi.y]
for idx_t in range(2000):
...
# kp = 0.1, Kd = 0.1, Ki =
audi = Car(length=2.3, y=0.3)
p_controller3 = PIDController(kp=0.8, ki=0.1, kd=0.4, ts=dt)
y_cache3 = [audi.y]
for idx_t in range(200 0):
...
#Plotting
plt.plot(t_cache,y_cache,'r',label='ki = 0.001')
plt.plot(t_cache,y_cache2,'g',label='ki = 0.01')
plt.plot(t_cache,y_cache3,'k',label='ki = 0.1')
plt.xlabel('Time (s)')
plt.ylabel('y-plane (m)')
plt.grid()
plt.title('PID controller of varying Ki values, but fixed Kp= 0.8 & Kd = 0.4')
